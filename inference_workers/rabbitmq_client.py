...
class _AsyncConnection(object):
    ...
    def __init__(self, host, port, username, password, io_loop, timeout=10):
        self._parameter = ConnectionParameters(
            host=host,
            port=port,
...
class RabbitMQConnector(object):
    ...
    def __init__(self, host, port, username, password, io_loop=None):
        """
        ...
        self._io_loop = io_loop
        self._logger = logging.getLogger(__name__)
        self._publish_conn = _AsyncConnection(host, port, username, password, io_loop)
        self._receive_conn = _AsyncConnection(host, port, username, password, io_loop)
    ...
    def _create_channel(self, connection, return_callback=None,
        ...
        print("CREATE CHANNEL 2")
        return future

    def _exchange_declare(self, channel, exchange=None, exchange_type='fanout', **kwargs):
    ...
    def receive(self, exchange, routing_key, queue_name, handler, no_ack=False, prefetch_count=0,
        ...
        try:
            ...
            self.logger.info("[receive] exchange: %s; routing key: %s; queue name: %s", exchange, routing_key,
                             queue_name)
    ...
    def _process_message(self, unused_channel, basic_deliver, properties, body, exchange, handler=None,
                         return_callback=None, close_callback=None):
        try:
            result = yield handler(properties, body)
